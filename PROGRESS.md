...
> 基于 `SPEC.md` 第8章开发计划实现，最后更新：2026-06-24
...
- **外部图片上传**：URL 图片通过 `fetch` 下载后上传到后端 `/api/images/upload`，获取真实 `image_id`
...
| 端点 | 方法 | 说明 |
|------|------|------|
| `/api/images/gallery/:id` | PUT | 更新图片名称和分类（支持部分更新） |
| `/api/images/gallery/:id` | DELETE | 删除图片记录 + 物理文件 + 缩略图 |
